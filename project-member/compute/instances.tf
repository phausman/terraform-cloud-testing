...
resource "openstack_compute_instance_v2" "basic_instance" {
    ...
    flavor_id = "${var.flavor_m1_small_id}"
...
resource "openstack_compute_floatingip_associate_v2" "myip" {
    ...
    fixed_ip = "${openstack_compute_instance_v2.instance-with-multiple-networks.network.1.fixed_ip_v4}"
    depends_on = [var.project_member_role_assignment]
}
...
resource "openstack_compute_instance_v2" "instance-with-multiple-ephemeral-disks" {
    ...
    image_id = "${var.cirros_image_id}"
    flavor_id = "${var.flavor_m1_medium_id}"
    key_pair = "${openstack_compute_keypair_v2.test-keypair.name}"
    security_groups = ["default", "secgroup"]
